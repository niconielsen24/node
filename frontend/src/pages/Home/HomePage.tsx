...
export default function HomePage() {
  ...
  const handleCreateUser = async (name: string) => {
    ...
    try {      
      const response = await caller.postUser(name);
  ...
  return (
    <section className="min-h-screen bg-zinc-950 text-white flex items-center justify-center px-8 py-20 font-mono w-screen">
      <div className="flex items-start w-full max-w-5xl mx-auto">
        ...
        {/* CreateUser panel — slides in beside main content */}
        <div
          className={`transition-all duration-300 ease-out overflow-hidden flex-shrink-0 ${createUserVisible ? "w-80 opacity-100 ml-8" : "w-0 opacity-0 ml-0"}`}
        >
